...
def test_env_values() -> None:
    ...
    assert values["AQUATICY_MODEL"] == lm.LOCAL_MODELS[0].model_id
    assert values["AQUATICY_MODEL"].startswith("ollama_chat/")
    assert values["AQUATICY_API_BASE"] == "http://localhost:11434"


def test_env_values_adds_the_prefix_to_a_bare_name() -> None:
    assert lm.env_values("qwen2.5:14b")["AQUATICY_MODEL"] == "ollama_chat/qwen2.5:14b"


def test_env_values_keeps_a_full_id() -> None:
    assert lm.env_values("ollama_chat/eigenes:tag")["AQUATICY_MODEL"] == "ollama_chat/eigenes:tag"


def test_env_model_id_resolves_in_litellm() -> None:
    """Die erzeugte ID muss LiteLLM auch wirklich zuordenbar sein."""
    from aquaticy.config import resolve_model

    assert resolve_model(lm.env_values(lm.LOCAL_MODELS[0])["AQUATICY_MODEL"]) == "ollama_chat"
...
def test_verify_rejects_a_model_that_only_talks(monkeypatch: pytest.MonkeyPatch) -> None:
    """Genau der Fall, der aquaticy unbrauchbar macht: Antwort ohne Werkzeugaufruf."""
    monkeypatch.setattr(
...
def test_vision_env_values() -> None:
    assert lm.vision_env_values("llava:7b") == {"AQUATICY_VISION_MODEL": "ollama_chat/llava:7b"}
    assert lm.vision_env_values(lm.VISION_MODELS[0])["AQUATICY_VISION_MODEL"].startswith(
        "ollama_chat/"
    )
