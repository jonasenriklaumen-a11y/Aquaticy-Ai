...
from aquaticy.models import SearchResult, domain_of
from aquaticy.queries import fuse
...
def _search_searxng(query: str, options: SearchOptions) -> list[SearchResult]:
    ...
    """
    base = (options.instance_url or os.environ.get("AQUATICY_SEARXNG_URL", "")).strip()
    if not base:
        raise SearchError(
            "AQUATICY_SEARXNG_URL fehlt. Trage die Adresse deiner SearXNG-Instanz ein, "
