...
aquaticy soll auch Fragen beantworten koennen, die nicht im Web stehen: "laeuft
...
* **Nur die Frage "antwortet da etwas".** aquaticy klopft an, liest den Titel
...
#: Ports, die aquaticy kennt, mit dem, was dort ueblicherweise laeuft.
...
#: Groesstes Netz, das aquaticy am Stueck durchgeht. Ein /16 waere 65.000
...
def in_container() -> bool:
    """Laeuft Aquaticy AI in einem Container?"""
...
def container_hint(subnet: str = "") -> str:
    """Sagt es, wenn aquaticy im Container am eigenen Netz vorbeisieht.
...
def own_subnet() -> str:
    ...
    aquaticy nicht raten, welches Netz gemeint ist, und fragt lieber nach.
    """
    from aquaticy.web import lan_address
...
def parse_subnet(subnet: str) -> ipaddress.IPv4Network:
    ...
    if "/" not in text:
        ...
        if len(parts) == 3:  # "192.168.1" -> ganzes /24
            ...
            raise ValueError(f"Damit kann Aquaticy AI nichts anfangen: {subnet}")
    network = ipaddress.ip_network(text, strict=False)
    if not isinstance(network, ipaddress.IPv4Network):
        raise ValueError("Nur IPv4-Netze.")
    if not is_private_net(network):
        raise NotPrivate(
            f"{network} ist kein privates Netz. Aquaticy AI durchsucht nur das eigene "
...
def find_port(port: int, subnet: str = "") -> list[str]:
    ...
    Damit findet aquaticy Home Assistant (8123) oder Ollama (11434), ohne das
