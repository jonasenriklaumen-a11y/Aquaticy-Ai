"""Kommandozeile von aquaticy: Chat-Loop, Slash-Befehle, Setup."""
...
from aquaticy.search import OPEN_ENGINES

app = typer.Typer(
    add_completion=False,
    no_args_is_help=False,
    help="aquaticy -- Aquaticy AI, der Rechercheagent fuer die Kommandozeile.",
    context_settings={"help_option_names": ["-h", "--help"]},
)
console = Console()

#: Aquaticy ist auf zwei Anbieter spezialisiert -- mehr dazu in
#: `aquaticy/system.py`. Lokal bleibt lokal: Ollama ist kein Anbieter, sondern
...
# setup
# ---------------------------------------------------------------------------
# Die Testanfragen selbst stehen in aquaticy/probe.py -- die Web-Einstellungen
...
def setup_command(
    env_file: Path | None = typer.Option(
        None, "--env-file", help="Zieldatei fuer die .env (Default: ~/.config/aquaticy/.env)."
    ),
    ...
    if backend == "duckduckgo":
        ...
        searxng_url = typer.prompt(
            "  Adresse der Instanz", default=os.environ.get("AQUATICY_SEARXNG_URL", "")
    ...
    console.print(
        "  [dim]Aquaticy AI kann jede Anfrage in Teilfragen zerlegen und parallel "
    ...
    if not no_probe:
        ...
        if not (llm_ok and search_ok):
            console.print(
                ...
                "mit [bold]aquaticy setup[/bold] korrigieren."
            )

    # -- Schreiben ---------------------------------------------------------
    values: dict[str, str] = {
        "AQUATICY_MODEL": model,
        "AQUATICY_SEARCH_BACKEND": backend,
        "AQUATICY_LOCATION": location,
        "AQUATICY_LANG": lang,
        "AQUATICY_COUNTRY": country,
        "AQUATICY_SUBAGENTS_AUTO": "true" if subagents_on else "false",
    }
    ...
    if engines:
        values["AQUATICY_SEARCH_ENGINES"] = engines
    if searxng_url:
        values["AQUATICY_SEARXNG_URL"] = searxng_url
    if api_base:
        values["AQUATICY_API_BASE"] = api_base
    ...
    console.print(
        Panel.fit(
            ...
            "Jetzt loslegen:\n"
            '  [bold]aquaticy[/bold]                     Chat starten\n'
            '  [bold]aquaticy web[/bold]                 Oberflaeche im Browser\n'
            '  [bold]aquaticy "deine Frage"[/bold]       einmalige Recherche',
            title="fertig",
            border_style="green",
        )
    )
...
def config_command() -> None:
    ...
    table = Table(title="aquaticy-Konfiguration", show_header=False, title_justify="left")
    ...
    if problems:
        ...
        console.print("Behebe sie mit [bold]aquaticy setup[/bold].")
    else:
...
def web_command(
    ...
    Ohne Argumente hoert aquaticy nur auf dem eigenen Rechner. `--lan` macht ihn
...
def google_command(
    ...
    Danach beantwortet Aquaticy AI Fragen nach Terminen und Mails aus dem eigenen
    ...
    holt Aquaticy sich das Recht gar nicht erst --, und vor jeder Aenderung
    fragt er nach.
    """
    load_env()
    settings = get_settings()

    from aquaticy.google import GoogleError, TokenStore, code_from, consent_url, exchange_code
    from aquaticy.web import DEFAULT_PORT, google_client
    ...
    from aquaticy.google import Google
    ...
    written = write_env_file(
        {
            ...
            "AQUATICY_GOOGLE_WRITE": "true" if write else "false",
    ...
    console.print(
        "[dim]Aquaticy darf hier "
        + (
            "auch Termine anlegen/aendern und Entwuerfe schreiben -- "
            "verschickt wird nie etwas, und vor jeder Aenderung wird gefragt."
            if write
            else "nur lesen. Aendern erlaubst du mit: aquaticy google --aendern"
        )
        + "[/dim]"
    )
    console.print(
        "[dim]Probier es aus:[/dim]\n"
        '  [bold]aquaticy "was habe ich diese Woche vor"[/bold]\n'
        '  [bold]aquaticy "habe ich eine Mail von der Bahn bekommen"[/bold]'
    )
    console.print("[dim]Wieder loswerden: aquaticy google --trennen[/dim]")
...
def connect_ha_command(
    ...
    """Verbindet aquaticy mit Home Assistant -- sucht die Instanz selbst.

    Danach kann aquaticy Fragen ueber das eigene Haus beantworten: Temperaturen,
    Lichter, Fenster, Anwesenheit.
    """
    from aquaticy.homeassistant import HomeAssistant, HomeAssistantError, discover, normalize_url
    ...
    if not token:
        console.print(
            ...
            "Zugriffstokens[/bold]\n"
            "  → [bold]Token erstellen[/bold], Namen vergeben (z.B. Aquaticy AI), kopieren.[/dim]"
        )
    ...
    # -- 4. Schalten? ------------------------------------------------------
    console.print(
        "\n[bold]Darf Aquaticy AI auch schalten?[/bold]\n"
    ...
    values = {
        "AQUATICY_HA_URL": url,
        "HA_TOKEN": token,
        "AQUATICY_HA_CONTROL": str(control).lower(),
    }
    target = env_file or find_env_file() or DEFAULT_ENV_PATH
    written = write_env_file(values, target)
    ...
    console.print(
        Panel.fit(
            f"Gespeichert in [cyan]{written}[/cyan]\n\n"
            "Probier es aus:\n"
            '  [bold]aquaticy "wie warm ist es im Wohnzimmer"[/bold]\n'
            '  [bold]aquaticy "welche Lichter sind gerade an"[/bold]'
            + ("\n  [bold]aquaticy \"mach das Licht im Flur aus\"[/bold]" if control else ""),
            title="fertig",
            border_style="green",
        )
    )
...
def lan_command(
    ...
    from aquaticy.lan import NotPrivate, own_subnet, scan

    settings = get_settings()
    target = subnet or settings.lan_subnet or own_subnet()
    if not target:
        console.print("[red]Kein eigenes Netz erkennbar.[/red]")
        console.print("[dim]Gib es an: aquaticy lan --subnet 192.168.1.0/24[/dim]")
        raise typer.Exit(code=1)
...
def install_browser_command() -> None:
    ...
    try:
        ...
        console.print('  [bold]uv tool install --with playwright aquaticy[/bold]')
        console.print("  [dim]oder: pip install \'aquaticy[browser]\'[/dim]")
...
def _run_vision_setup(
    ...
    console.print("\n[bold]5. Vision-Modell[/bold] [dim](fuer aquaticy --image und /image)[/dim]")
    if main_can_see:
        console.print(
    ...
    if not (model_name or assume_yes) and not typer.confirm(
        ...
        console.print("  [dim]Uebersprungen. Spaeter: aquaticy install-model --vision-only[/dim]")
        return ""
...
def _run_local_setup(model_name: str = "", assume_yes: bool = False) -> str:
    ...
    if works:
        ...
        console.print(
            ...
            "Nimm ein anderes aus der Liste -- Aquaticy AI wuerde sonst aus dem "
...
def install_model_command(
    ...
    from aquaticy.local_model import DEFAULT_OLLAMA_URL, env_values, vision_env_values
    ...
    if vision_only:
        ...
        from aquaticy.local_model import DUAL_MODELS
        ...
        if subagent_model or (subagents and not yes):
            ...
            if helper:
                values["AQUATICY_SUBAGENT_MODEL"] = helper

    target = env_file or find_env_file() or DEFAULT_ENV_PATH
    written = write_env_file(values, target)
    reset_settings_cache()

    lines = [f"{key.removeprefix('AQUATICY_')}: [bold cyan]{value}[/bold cyan]"
    ...
    console.print(
        Panel.fit(
            ...
            "Loslegen:\n"
            "  [bold]aquaticy[/bold]                     Chat\n"
            "  [bold]aquaticy web[/bold]                 Oberflaeche im Browser\n"
            '  [bold]aquaticy "deine Frage"[/bold]       einmalige Recherche\n'
            + (
                "  [bold]aquaticy --image foto.jpg[/bold]  Bild als Ausgangspunkt"
                if "AQUATICY_VISION_MODEL" in values
                else ""
            ),
            title="fertig",
            border_style="green",
...
def _warn_if_unconfigured(settings: Settings) -> None:
    ...
    if problems:
        console.print(
            Panel.fit(
                "\n".join(problems)
                + "\n\nRichte aquaticy mit [bold]aquaticy setup[/bold] ein."
                + "\n[dim]Keinen API-Key? [bold]aquaticy install-model[/bold] richtet ein "
...
def _record_turn(turns: list, question: str, result) -> None:
    from aquaticy.export import Turn
...
def _do_export(turns: list, fmt: str, settings: Settings) -> None:
    from aquaticy.export import export
...
def chat_command(
    ...
    from aquaticy.render import ChatRenderer
...
#: Was aquaticy als Bild akzeptiert.
...
def _resolve_image(path: Path) -> Path | None:
    ...
    Zeigt *path* auf einen Ordner, sucht aquaticy darin nach Bildern: bei genau
    ...
    *image* darf eine Datei oder ein Ordner sein -- siehe :func:`_resolve_image`.
    ...
    try:
        ...
        console.print(
            ...
            "richtet [bold]aquaticy install-model --vision-only[/bold] ein.[/dim]"
        )
        return None
...
def _handle_slash(
    ...
    if command == "help":
        ...
        console.print(
            ...
            "recherchiert Aquaticy ohnehin mit allen eingestellten Agenten.[/dim]"
        )
...
def _show_memory(settings: Settings) -> None:
    ...
    if not settings.memory_enabled:
        ...
        console.print("[dim]Einschalten mit AQUATICY_MEMORY=true in der .env.[/dim]")
        return
    from aquaticy.memory import human_size
...
def _command_names() -> set[str]:
    ...
    prompt auseinander: `aquaticy google` landete im Chat, statt sich zu
...
def _unknown_command(name: str) -> None:
    ...
    console.print(
        f'[dim]War das als Frage gemeint? Dann in Anfuehrungszeichen: aquaticy "{name}"[/dim]'
    )


def main() -> None:
    """Einstiegspunkt: `aquaticy` und `aquaticy "Frage"` landen im Chat.

    Der Befehl heisst aquaticy; `aquaticy` bleibt als zweiter Name bestehen,
