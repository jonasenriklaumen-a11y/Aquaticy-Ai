"""Auftraege: Fragen, die Aquaticy von selbst stellt.
...
genau das: eine gespeicherte Frage mit einem Rhythmus. Aquaticy stellt sie sich
...
class Scheduler:
    ...
    def start(self) -> None:
        if self._thread is not None:
            return
        self._thread = threading.Thread(target=self._loop, name="aquaticy-jobs", daemon=True)
        self._thread.start()
